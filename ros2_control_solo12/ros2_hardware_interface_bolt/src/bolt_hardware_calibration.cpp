...
int main()
{
    ...
    Eigen::Matrix<double,12,1> zero6=Eigen::Matrix<double,12,1>::Zero();
    ...
    // Define controller to calibrate the joints from yaml file.
    auto calib_ctrl = JointCalibratorFromYamlFile(
        CONFIG_SOLO12_ONE_JOINT_YAML, robot->joints);
    Eigen::Matrix<double,1,1> zero1=Eigen::Matrix<double,1,1>::Zero();
    calib_ctrl->UpdatePositionOffsets(zero1);
    // Initialize simple pd controller.
    Vector1d torques;
    */
